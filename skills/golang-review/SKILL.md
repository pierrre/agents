...
Acceptable proofs: `i` comes from `for i := range s`; a preceding `if i < len(s)` (plus `i >= 0` when `i` is signed and untrusted); or `i` is a constant known in-bounds.
...
`a / b` and `a % b` on integers panic when `b == 0`; float division yields `±Inf`/`NaN` (logic bug). Guard the divisor:
...
- **`sync/atomic`** — a single primitive (counter, flag, timestamp). Structs, interfaces, slices, and maps are *not* single-word; use a mutex.
...
```go
// ✗ shared loop counter — goroutines read the same i the loop mutates
for i := 0; i < n; i++ { go func() { fmt.Println(i) }() }
// ✓ pass i as a parameter (or shadow: i := i)
...
| Shared variable (incl. loop counter, outer `err`) read/written by >1 goroutine | Pass as param / shadow with `:=` / guard with mutex or `sync/atomic` |
...
`nilness`, `nilerr`, `errcheck`, `govet`, `staticcheck` (SA series), `gosec` catch many of these mechanically. The `-race` flag is a **runtime** detector (see §7), not static — it only catches races in executed code paths. `copyloopvar` (Go 1.22+ loop-var semantics) prevents the classic shared-counter race. → See `samber/cc-skills-golang@golang-lint`.
